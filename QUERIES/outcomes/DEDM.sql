...
-- DE / DM / DC combined outcomes -- union schema v1
...
cohort AS (
    ...
    -- admit_type sourced from SORLCUR_ADMT_CODE (as-of-term) for accuracy.
    ...
    --   DE  = all others (including JE/AT/H)
    ...
    SELECT
        spr.SPRIDEN_ID
            || '-'
            || CASE
                   WHEN lae.SORLCUR_ADMT_CODE = 'DM'
                    AND NVL(ftc_dm.ft_term_count, 0) >= 1  THEN 'DM'
                   WHEN lae.SORLCUR_ADMT_CODE = 'DE'
    ...
    AND (
        (   lae.SORLCUR_ADMT_CODE      IN ('DE','DM','JE','AT')
            AND NVL(sg.SGBSTDN_STYP_CODE, 'X') NOT IN ('C')
        )
        OR  sg.SGBSTDN_STYP_CODE = 'H'
    )
    AND lae.SORLCUR_ADMT_CODE != 'FR'
    GROUP BY
        spr.SPRIDEN_ID,
        sf.PIDM_A3,
        lae.SORLCUR_ADMT_CODE,
        sg.SGBSTDN_STYP_CODE,
        NVL(pc.prior_cum_hrs, 0),
        CASE WHEN lae.SORLCUR_ADMT_CODE = 'DM'
              AND NVL(ftc_dm.ft_term_count, 0) >= 1 THEN 'DM'
             WHEN lae.SORLCUR_ADMT_CODE = 'DE'
              AND NVL(ftc_de.ft_term_count, 0) = 2  THEN 'DC'
             ELSE 'DE' END,
        CASE WHEN lae.SORLCUR_ADMT_CODE = 'DM'
              AND NVL(ftc_dm.ft_term_count, 0) = 0
             THEN 1 ELSE 0 END,
        CASE WHEN lae.SORLCUR_ADMT_CODE = 'DE'
              AND NVL(ftc_de.ft_term_count, 0) >= 3
             THEN 1 ELSE 0 END
),
...
matriculation AS (
    ...
    -- within 4 term sequences of last DE/DM term (~one year + buffer
    -- to catch students returning the following Fall).
    -- Requires a non-NDUG SORLCUR major: student must have declared
    -- a real program, not just enrolled as non-degree.
    SELECT
        c.cohort_id,
...
last_dedm_term AS (
    ...
    -- for next_term_matriculation_flag (two-term gap tolerance).
    SELECT
        c.cohort_id,
        c.pidm,
...
next_term_mat AS (
    ...
    -- after last DE/DM term (two-term window for gap tolerance).
    SELECT
        ldt.cohort_id,
        MAX(CASE
...
FROM outcomes o
LEFT JOIN matriculation mat
    ON  mat.cohort_id = o.cohort_id
LEFT JOIN current_major cm
    ON  cm.cohort_id  = o.cohort_id
LEFT JOIN next_term_mat ntm
    ON  ntm.cohort_id = o.cohort_id
